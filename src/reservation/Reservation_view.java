...
public class Reservation_view extends Frame implements ActionListener,WindowListener,KeyListener {
	...
	public Reservation_view(Reservation_control rcontrol) {
		...
		// 上部パネルの中央パネルに 施設[施設名選択]チョイス[概要説明]ボタンを追加
	...
	public void actionPerformed(ActionEvent e) {
		...
		}else if (e.getSource() == buttonLog){ //ログイン,ログアウトボタン
			result = rcontrol.loginLogout(this);
		}else if (e.getSource() == buttonReservation){	//予約ボタン
			result = rcontrol.makeReservation(this);
		}else if (e.getSource() == buttonExplanation){	//概要ボタン
			result = rcontrol.explanation(choiceFacility.getSelectedItem());
		}else if (e.getSource() == buttonConfirm){	//予約確認ボタン
			result = rcontrol.confirmReservation(this);
		}else if (e.getSource() == buttonCancel){	//予約取り消しボタン
			result = rcontrol.deleteReservation(this);
		}
